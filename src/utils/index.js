 ...
 * @param appointmentId  约课id  required
 * @param callback  取消之后callback
 */
export const cancelAppointment = (appointmentId, callback = () => {}) => {
    ...
    const confirm = () => {
        close()
        Http.ajax(`/cancel-appointment`, params).then(res => {
            if (res.code === '0') {
                Toast.info(res.message || '取消成功', 2, null, false)
                callback()
            }
        })
    }
    ...
    render(<CancelAppointment visible onClose={close} onConfrim={confirm} />, div)
